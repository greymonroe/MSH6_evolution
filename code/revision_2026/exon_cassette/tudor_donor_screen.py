#!/usr/bin/env python3
"""
Comprehensive Tudor domain donor screen for plant MSH6.
Same pipeline as PWWP screen:
1. Download all Tudor proteins from InterPro (PF00567 + PF17089/Agenet)
2. UniProt -> EMBL mapping
3. NCBI batch fetch for coded_by exon structure + Tudor sequences
4. Compute exon break positions + extract domain sequences for phylogenetic comparison

For Tudor, sequence similarity is the primary signal (not exon breaks),
since plant MSH6 Tudor is mostly in a single exon.
"""

import csv, json, re, sys, time
from pathlib import Path
from io import StringIO
from collections import defaultdict
from Bio import Entrez, SeqIO
import requests

Entrez.email = 'greymonroe@gmail.com'
WORK = Path(__file__).parent


def fetch_interpro_list(pfam_ids):
    """Download proteins from InterPro for given Pfam IDs."""
    cache = WORK / 'interpro_tudor_proteins.json'
    if cache.exists():
        print('  Loading InterPro cache...', flush=True)
        with open(cache) as f:
            return json.load(f)

    proteins = []
    for pfam_id in pfam_ids:
        url = f'https://www.ebi.ac.uk/interpro/api/protein/UniProt/entry/pfam/{pfam_id}/?page_size=200&format=json'
        page = 0
        while url:
            page += 1
            if page % 25 == 1:
                print(f'  {pfam_id} page {page} ({len(proteins)} total)...', flush=True)
            try:
                r = requests.get(url, timeout=60)
                r.raise_for_status()
                data = r.json()
            except Exception as e:
                print(f'    Page {page} failed: {e}', flush=True)
                time.sleep(2)
                continue

            for entry in data.get('results', []):
                meta = entry.get('metadata', {})
                acc = meta.get('accession', '')
                org = meta.get('source_organism', {})
                for loc_group in entry.get('entries', []):
                    for loc in loc_group.get('entry_protein_locations', []):
                        for frag in loc.get('fragments', []):
                            proteins.append({
                                'uniprot': acc,
                                'organism': org.get('scientificName', '?'),
                                'tax_id': org.get('taxId', 0),
                                'prot_len': meta.get('length', 0),
                                'domain_start': frag.get('start', 0),
                                'domain_end': frag.get('end', 0),
                                'pfam': pfam_id,
                            })
            url = data.get('next')
            time.sleep(0.2)

    with open(cache, 'w') as f:
        json.dump(proteins, f)
    return proteins


def uniprot_to_embl(uniprot_accs, batch_size=500):
    mapping = {}
    for i in range(0, len(uniprot_accs), batch_size):
        batch = uniprot_accs[i:i + batch_size]
        bn = i // batch_size + 1
        total = (len(uniprot_accs) + batch_size - 1) // batch_size
        if bn % 10 == 1:
            print(f'  UniProt mapping batch {bn}/{total}...', flush=True)
        try:
            r = requests.post('https://rest.uniprot.org/idmapping/run',
                              data={'from': 'UniProtKB_AC-ID', 'to': 'EMBL-GenBank-DDBJ_CDS',
                                    'ids': ','.join(batch)}, timeout=30)
            r.raise_for_status()
            job_id = r.json()['jobId']
            time.sleep(1)
            for _ in range(30):
                r2 = requests.get(f'https://rest.uniprot.org/idmapping/status/{job_id}', timeout=30)
                status = r2.json()
                if 'results' in status or 'failedIds' in status:
                    break
                if status.get('jobStatus') == 'FINISHED':
                    break
                time.sleep(1)
            r3 = requests.get(f'https://rest.uniprot.org/idmapping/results/{job_id}?size=500', timeout=30)
            r3.raise_for_status()
            for res in r3.json().get('results', []):
                uni = res.get('from', '')
                embl = res.get('to', '')
                if uni and embl and uni not in mapping:
                    mapping[uni] = embl
            time.sleep(0.3)
        except Exception as e:
            print(f'    Mapping batch failed: {e}', flush=True)
            time.sleep(2)
    return mapping


def parse_coded_by(coded_by_str):
    parts = re.findall(r'(\d+)\.\.(\d+)', coded_by_str)
    if not parts:
        return None
    exons = []
    cum_nt = 0
    for i, (s, e) in enumerate(parts):
        nt = int(e) - int(s) + 1
        if nt < 0:
            return None
        aa_s = cum_nt // 3 + 1
        cum_nt += nt
        aa_e = cum_nt // 3
        exons.append({'num': i + 1, 'start': aa_s, 'end': aa_e, 'len': aa_e - aa_s + 1})
    total_aa = cum_nt // 3
    if total_aa < 50:
        return None
    return exons, total_aa


def batch_fetch_ncbi(accessions, batch_size=200):
    results = {}
    n_batches = (len(accessions) + batch_size - 1) // batch_size
    for i in range(0, len(accessions), batch_size):
        batch = accessions[i:i + batch_size]
        bn = i // batch_size + 1
        if bn % 10 == 1:
            print(f'  NCBI batch {bn}/{n_batches} ({len(results)} results)...', flush=True)
        try:
            handle = Entrez.efetch(db='protein', id=','.join(batch),
                                   rettype='gp', retmode='text')
            text = handle.read()
            handle.close()
            time.sleep(0.5)
        except Exception as e:
            print(f'    Batch {bn} failed: {e}', flush=True)
            time.sleep(2)
            continue
        try:
            for record in SeqIO.parse(StringIO(text), 'genbank'):
                acc = record.id
                seq = str(record.seq)
                coded_by = None
                gene_name = ''
                product = ''
                for feat in record.features:
                    if feat.type == 'CDS':
                        coded_by = feat.qualifiers.get('coded_by', [None])[0]
                        gene_name = feat.qualifiers.get('gene', [''])[0]
                        product = feat.qualifiers.get('product', [''])[0]
                        break

                exons = None
                if coded_by and 'join' in coded_by:
                    result = parse_coded_by(coded_by)
                    if result:
                        exons, _ = result

                # Find Tudor/Agenet region
                tudor_s = tudor_e = None
                for feat in record.features:
                    if feat.type == 'Region':
                        name = feat.qualifiers.get('region_name', [''])[0].upper()
                        if 'TUDOR' in name or 'AGENET' in name:
                            s = int(feat.location.start) + 1
                            e = int(feat.location.end)
                            if tudor_s is None or s < tudor_s:
                                tudor_s = s
                            if tudor_e is None or e > tudor_e:
                                tudor_e = e

                tudor_seq = ''
                if tudor_s and tudor_e and len(seq) >= tudor_e:
                    tudor_seq = seq[tudor_s - 1:tudor_e]

                results[acc] = {
                    'exons': exons,
                    'prot_len': len(seq),
                    'tudor_s': tudor_s, 'tudor_e': tudor_e,
                    'tudor_seq': tudor_seq,
                    'gene': gene_name, 'product': product,
                    'full_seq': seq,
                }
        except Exception as e:
            print(f'    Parse error: {e}', flush=True)
    return results


def main():
    print('=== Phase 1: InterPro Tudor protein list ===', flush=True)
    interpro = fetch_interpro_list(['PF00567', 'PF17089'])
    print(f'  Total entries: {len(interpro)}', flush=True)

    by_uni = {}
    for p in interpro:
        acc = p['uniprot']
        if acc not in by_uni:
            by_uni[acc] = p
        else:
            if p['domain_start'] < by_uni[acc]['domain_start']:
                by_uni[acc]['domain_start'] = p['domain_start']
            if p['domain_end'] > by_uni[acc]['domain_end']:
                by_uni[acc]['domain_end'] = p['domain_end']
    print(f'  Unique UniProt accessions: {len(by_uni)}', flush=True)

    print('\n=== Phase 2: UniProt -> EMBL mapping ===', flush=True)
    cache_map = WORK / 'uniprot_to_embl_tudor_map.json'
    if cache_map.exists():
        print('  Loading mapping cache...', flush=True)
        with open(cache_map) as f:
            uni_to_embl = json.load(f)
    else:
        uni_to_embl = uniprot_to_embl(list(by_uni.keys()))
        with open(cache_map, 'w') as f:
            json.dump(uni_to_embl, f)
    print(f'  Mapped {len(uni_to_embl)} / {len(by_uni)} UniProt -> EMBL', flush=True)

    print('\n=== Phase 3: NCBI batch fetch ===', flush=True)
    embl_accs = list(set(uni_to_embl.values()))
    print(f'  Unique EMBL accessions: {len(embl_accs)}', flush=True)

    cache_ncbi = WORK / 'ncbi_tudor_results.json'
    if cache_ncbi.exists():
        print('  Loading NCBI cache...', flush=True)
        with open(cache_ncbi) as f:
            ncbi_results = json.load(f)
    else:
        ncbi_results = batch_fetch_ncbi(embl_accs)
        with open(cache_ncbi, 'w') as f:
            json.dump(ncbi_results, f)
    print(f'  Got NCBI data for {len(ncbi_results)} proteins', flush=True)

    print('\n=== Phase 4: Computing Tudor exon breaks + extracting sequences ===', flush=True)

    output_rows = []
    tudor_seqs = []

    for uni_acc, info in by_uni.items():
        embl_acc = uni_to_embl.get(uni_acc)
        if not embl_acc:
            continue
        ncbi = ncbi_results.get(embl_acc)
        if not ncbi:
            continue

        tud_s = ncbi.get('tudor_s') or info['domain_start']
        tud_e = ncbi.get('tudor_e') or info['domain_end']
        exons = ncbi.get('exons')

        break_rel = None
        n_exons_tudor = 0
        if exons and tud_s and tud_e:
            overlapping = [e for e in exons if e['end'] >= tud_s and e['start'] <= tud_e]
            n_exons_tudor = len(overlapping)
            if n_exons_tudor >= 2:
                break_rel = overlapping[0]['end'] - tud_s

        tudor_seq = ncbi.get('tudor_seq', '')
        if not tudor_seq and tud_s and tud_e:
            full = ncbi.get('full_seq', '')
            if len(full) >= tud_e:
                tudor_seq = full[tud_s - 1:tud_e]

        output_rows.append({
            'uniprot': uni_acc,
            'embl': embl_acc,
            'organism': info['organism'],
            'tax_id': info['tax_id'],
            'prot_len': ncbi.get('prot_len', info['prot_len']),
            'gene': ncbi.get('gene', ''),
            'product': ncbi.get('product', ''),
            'tudor_start': tud_s,
            'tudor_end': tud_e,
            'tudor_len': tud_e - tud_s + 1 if tud_s and tud_e else 0,
            'has_genomic_exons': exons is not None,
            'n_total_exons': len(exons) if exons else 1,
            'n_exons_in_tudor': n_exons_tudor,
            'break_from_tudor_start': break_rel if break_rel is not None else '',
        })

        if tudor_seq:
            brk = f'break+{break_rel}' if break_rel is not None else 'no_break'
            tudor_seqs.append(f'>{embl_acc}|{uni_acc} {info["organism"]} {ncbi.get("gene","")} {brk}\n{tudor_seq}')

    out_tsv = WORK / 'tudor_all_proteins_breaks.tsv'
    with open(out_tsv, 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=[
            'uniprot', 'embl', 'organism', 'tax_id', 'prot_len', 'gene', 'product',
            'tudor_start', 'tudor_end', 'tudor_len',
            'has_genomic_exons', 'n_total_exons', 'n_exons_in_tudor',
            'break_from_tudor_start',
        ], delimiter='\t')
        w.writeheader()
        w.writerows(output_rows)

    out_fasta = WORK / 'tudor_domain_sequences.fasta'
    with open(out_fasta, 'w') as f:
        f.write('\n'.join(tudor_seqs))

    print(f'\nWrote {len(output_rows)} rows to {out_tsv}')
    print(f'Wrote {len(tudor_seqs)} Tudor sequences to {out_fasta}')

    with_exons = [r for r in output_rows if r['has_genomic_exons']]
    with_break = [r for r in output_rows if r['break_from_tudor_start'] != '']
    print(f'\nWith genomic exon data: {len(with_exons)}')
    print(f'With Tudor exon break: {len(with_break)}')

    if with_break:
        from collections import Counter
        breaks = Counter(int(r['break_from_tudor_start']) for r in with_break)
        print(f'\nBreak position distribution (top 20):')
        for pos, count in breaks.most_common(20):
            bar = '#' * min(count, 50)
            print(f'  +{pos:3d} aa: {count:5d} {bar}')

    print('\n=== Done ===')


if __name__ == '__main__':
    main()
